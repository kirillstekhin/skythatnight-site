...
/* ── ГЕЙТ МЕСТА ПЕРЕД ЧЕКАУТОМ (25.08.2026) ──
   Место вводится ДВАЖДЫ: здесь (координаты → design-код) и отдельным словом на чекауте
   Stripe. Пример (вымышленный): покупатель ушёл в оплату с нетронутым конфигуратором и
   написал место только там — на постере YORK над координатами дефолтного Лондона. Дефолт не
...
function attachGeocode() {
  /* ⚠ БАГ (исправлен 14.08.2026). Раньше координаты применялись ТОЛЬКО по клику на подсказку.
     Пример (вымышленный): человек набирает «Boston», подсказку не нажимает и уходит
     дальше — в поле стоит Boston, а state хранит координаты предыдущего места (скажем,
     Санторини из пресета другой страницы). В дизайн-код уходят СТАРЫЕ координаты, и на
     постере под словом BOSTON печатаются координаты Санторини. Поймать это успевает
     только окно тишины.
  ...
  /* ⚠ Автовыбор БЕЗ клика обязан быть консервативным. Пример (вымышленный, выдача сверена
     15.09.2026): на запрос «Boston» геокодер первым отдаёт Бостон в Массачусетсе, а покупатель
     почти наверняка имеет в виду Boston в Линкольншире — доставка бесплатна только по UK, и все заказы
  ...
  /* ⚠ БРИТАНСКИЕ МЕСТА ПЕРВЫМИ (14.08.2026). Пример (вымышленный): на запрос «Boston» геокодер
     отдаёт ТОЛЬКО американские города — британский Boston в общей выдаче не появляется, и
  ...
  /* ⚠ «CITY, UK» ЛОМАЛ ПРИВЯЗКУ (08.09.2026). open-meteo ищет по имени места: «York, UK»,
